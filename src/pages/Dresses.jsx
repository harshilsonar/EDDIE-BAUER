...
const Dresses = () => {
  ...
  const fetchProduct = () => {
    ...
    axios.get(`http://localhost:3000/Dresses`)
      ...
      .catch(() => {
        setError('Something went wrong while fetching dresses');
        setLoading(false);
      });
  };
  ...
  <h1 className="text-2xl sm:text-3xl font-bold mb-4 sm:mb-6 text-center">Dresses</h1>
      ...
      <div className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6">
        {filteredProducts.map((item, index) => (
          <div key={item.id} className="bg-white border rounded-lg p-3 sm:p-4 shadow text-center hover:shadow-md transition-shadow">
            <Link to={`/product/Dresses/${item.id}`}>
              <img 
